...
public class Inventory : MonoBehaviour {
	...
	public int slotXpadding = 1;//as per slotsize 
	public int slotYpadding = 1;//as per slotsize 
	...
	{	
		...
		for (int y = 0; y < slotY; ++y) 
		{
			for (int x = 0; x < slotX; ++x) 
			{
				...
				//Debug.Log(currentevent.clickCount);
				...
				}else
				{
					...
					if(slotRect.Contains(currentevent.mousePosition))//check if mouse hover over the box
					{
						...
						{
							Vector2 diff = currentevent.mousePosition - mouseprevposition;
							slottempoffset = diff;
